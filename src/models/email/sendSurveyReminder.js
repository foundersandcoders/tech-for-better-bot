...
const sendSurveyReminder = record => {
  ...
  const html = `
  <p>Hi there,</p>
  <p>Just a gentle reminder that the deadline for applying to the programme is ${userResearchDeadline
    .split("-")
    .join(" ")}. In order for your project to be up for selection by the London cohort, we'll need your application, 
including the results of your user research before that date.</p>
  <p>Here's the <a href="${researchSurveyUrl}?prefill_Email=${
    record.fields["Email"]
  }&prefill_application_id=${record.id}&prefill_Name=${
    record.fields["Name"]
  }">application form</a>, in case you need it again.</p>
